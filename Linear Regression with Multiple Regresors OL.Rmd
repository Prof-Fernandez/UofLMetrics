...
p <- .5 # set correlation parameter
x1 <- rnorm(n)  # independent variable
x2 <- rnorm(n)+p*x1  # omitted variable
beta1 <- 1  # true coefficient for x1
beta2 <- 2  # true coefficient for x2
epsilon <- rnorm(n)  # error term
y <- 1 + beta1*x1 + beta2*x2 + epsilon  # dependent variable
lm_perfect <-lm(y~x1+x2)
# Model with the omitted variable
lm_omitted <- lm(y ~ x1)
# Model without the omitted variable
p <- 0
x2 <- rnorm(n)+p*x1  # independent variable
y <- 1 + beta1*x1 + beta2*x2 + epsilon  # dependent variable
lm_omitted2 <- lm(y ~ x1)
# Model with the omitted variable
p<- -0.5
x2 <- rnorm(n)+p*x1  # independent variable
y <- 1 + beta1*x1 + beta2*x2 + epsilon  # dependent variable
lm_omitted3 <- lm(y ~ x1) # Model with the omitted variable
lm_no_omitted <- lm(y ~ x1+x2)
modelsummary(list("Truth"=lm_perfect,"p=0.5"=lm_omitted,"p=0"=lm_omitted2,"p=-0.5"=lm_omitted3,"Both"=lm_no_omitted), gof_map = c("r.squared","nobs"))
```
...
lm_omitted3 <- lm(y ~ x1) # Model with the omitted variable
lm_no_omitted <- lm(y ~ x1+x2)
modelsummary(list("Truth"=lm_perfect,"p=0.5"=lm_omitted,"p=0"=lm_omitted2,"p=-0.5"=lm_omitted3,"Both"=lm_no_omitted),stars = TRUE, gof_map = c("r.squared","nobs"))
```
...
modelsummary(list(food.ols,food.hetero), gof_map = c("r.squared","nobs"), stars = TRUE)
```
...
__Notice the lower effect of STR in the second regression.__
...
For multiple regressions we need to include a new assumptions

1. The error term $u_i$ has a mean of zero, $E[u_i]=0$.
2. The error term is independent of the X's, $E[u_i|X_{1i}, \dots, X_{ki}]=0$.
3. The error term has a constant variance, $Var[u_i|X_{1i}, \dots, X_{ki}]=\sigma^2$.
4. The variables $Y_i$ and $X_{1i}, \dots, X_{ki}$ are independently identically distributed (i.e. they are a random sample)
5. The relationship between $Y_i$ and $X_{1i}, \dots, X_{ki}$ is linear with respect to the parameters
6. There are no large outliers.
7. For testing reasons, we do assume the error terms are normally distributed.
8. __No Perfect Multicollinearity__

---
## No Perfect Multicollinearity (1)

The regressors exhibit perfect multicollinearity if one of the regressors is a linear function of the other regressor. Example:
...
---
## No Perfect Multicollinearity (2)
...
> Imperfect multicollinearity means that two or more regressors are highly correlated. It differs from perfect multicollinearity in that they don't have to be exact linear functions of each other.
...
The reason is that OLS rewards new information. Two variables that are highly correlated provide redundant information. Redundant information decreases the degrees freedom without decreasing the variance of the error. So our estimates become less precise

...
# Quick Review of Difference-in-Differences 
...
Will adding control variables affect diff-in-diff estimates if treatment assignment was random?

- __Answer__ = Not unless you've added 'bad controls', which are controls that also are affected by treatment.
...
- __Control Variables__ that are exogenous will just improve precision, but shouldn't affect estimates
...
If you find ex-ante differences in treated and untreated, is internal validity gone?

- __Answer__ = Not necessarily but it could suggest non-random assignment of treatment that is problematic. E.g. observations with characteristic 'z' are more likely to be treated and observations with this characteristic are also more likely to be trending differently for other reasons
...
How are triple differences helpful and reducing concerns about violation of parallel trends assumption?
 ...
 __Answer__ = Triple differences introduce a second control group. This second control group should have a similar trend to the initial control group, but not be affected at all by the treatment. 
 
An "identification policeman" would just need a story about why treated might be trending differently after event for other reasons. Now, he/she would need story about why that different trend would be particularly true for the subset of firms/individuals that are more sensitive to treatment
...
A researcher is interested in how this treatment (i.e. the cutoff) affects the outcome variable of interest, $y$.
...
- Has a similar flavor to diff-in-diff natural experiment setting in that you can illustrate identification with a figure
- Plot outcome y against independent variable that determines treatment assignment, x.
- Should observe a sharp discontinuous change in y at the cutoff value of x.
...
- RDD has some key differences. 
    ...
    - When treatment only depends on x (what I'll later call "sharp RDD", there is no overlap in treatment & controls; i.e. we never observe the same x for a treatment unit and a control unit)
...
- Assignment to treatment and control isn't random, but whether an _individual_ observation is treated is assumed to be random.
  - i.e. the researcher assumes that observations (e.g. firm, person, etc.) can't perfectly manipulate their x value
  - Therefore, whether an observation _x_ falls immediately above or below key cutoff _appears_ random
...
---
class: middle, center, inverse
# List of RDD papers and their Running Variables

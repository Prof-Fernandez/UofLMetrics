...
The relative increase in wages for an additional year of experience is given by the first partial derivative
...
In other words, we can minimize the OVB by including variables in the regression equation that are important and potentially correlated with other regressors.
...
* Since OLS estimation is a minimization of $SSR$ problem, every time a new regressor is added to a regression, the $R^2$ would be non-decreasing.
...
modelsummary::modelsummary(list(m1),output = "markdown",coef_rename = c("factor(gender)female" = "Female"),gof_map = c("nobs", "r.squared"))
```
</small>
]
...
stargazer(m1,m2,type="html",omit.stat = c("f","ser"), single.row = TRUE, style ="qje", covariate.labels = c("Female","Age","Education"), star.char = c("a","b","c"))
```
</small>
---
## Quadratic function
...
stargazer(m1,m2,m3,type="html",omit.stat = c("f","ser"), covariate.labels = c("Female","Age","Age Sq","Education"), star.char = c("a","b","c"))
```
...
CPSSW8$age2=CPSSW8$age*CPSSW8$age
m3 = lm(earnings ~ factor(gender)+age+age2+education, data=CPSSW8)
stargazer(m1,m2,m3,type="html",omit.stat = c("f","ser"), style = "qje", single.row = TRUE,covariate.labels = c("Female","Age","Age Sq","Education"), star.char = c("a","b","c"))
```
</small>
---
## Interaction term 
...
stargazer(m1,m2,m3,m4,type="html",omit.stat = c("f","ser"), single.row = TRUE, covariate.labels = c("Female","Female*Educ","Age","Age Sq","Education"),style = "qje" , star.char = c("a","b","c"))
```
</small>
---
...
stargazer(m3,m4,m5,type="html",covariate.labels = c("Female", "MidWest","South","West","Education*Female","Age","Age Squared","Education"),omit.stat = c("f","ser"), style = "qje", single.row=TRUE, star.char = c("a","b","c"))
```
</small>
---
## Log Transformation
...
stargazer(m4,m5,m6,type="html", covariate.labels = c("Age", "Age Squared","Education","Female","Midwest","West","South","Education*Female"),omit.stat = c("f","ser"), style = "qje", single.row=TRUE, star.char = c("a","b","c"))
```
</small>
---
class: center, middle, inverse
...
reg4 <- plm(Price ~ Quantity, data = data1, model = "within", effect = "individual", index = c("Location","Year"))
...
When this is not the case, the errors are said to be heteroscedastic, or to have heteroscedasticity, and this behavior will be reflected in the residuals $u_i$  estimated from a fitted model. 
...
library(fixest)
library(pander)
res <- feols(mrall ~ beertax, fatality.data, panel.id = "state")
res1 <- feols(mrall ~ beertax| year, fatality.data, panel.id = "state")
res2 <- feols(mrall ~ beertax| state+year, fatality.data, panel.id = "state")
etable(res,res1,res2, vcov = "cluster",dict = c("beertax"="Beer Tax"), postprocess.df = pandoc.table.return, style = 'rmarkdown')
```

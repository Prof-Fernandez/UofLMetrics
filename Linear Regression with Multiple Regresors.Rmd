...
$$\hat{\beta}_1 = \frac{\sum_i (X_i - \bar{X})(Y_i - \bar{Y})}{\sum_i (X_i - \bar{X})^2} = \frac{s_{XY}}{s_X^2}=\frac{COV(X,Y)}{Var(X)}$$
...
Notice the $n-2$ in the denominator? Since we estimated two parameters $\beta_0$ and $\beta_1$, we lost two degrees of freedom.
---
...
modelsummary(regress.results, coef_rename = c("str"="Student-Teacher Ratio"), gof_map = c("nobs", "r.squared"))
```
...
- Since all observations are randomly picked, then each observation should be independent.
...
- Randomization over the __whole__ distribution also eliminates sample selection issues.
---
...
$$wages_{i}=4.25+2.50Education_{i}+3Experience_{i}-.25Experience^2_{i}+e_{i}$$
...
```{r}
library(estimatr)
food.hetero <- lm_robust(food_exp ~ income, data = food, se_type = "stata")
```
...
Since wage is always positive perhaps we should consider log wages instead.

```{r echo=FALSE}
histogram(~log(CPSSW8$earnings) | CPSSW8$gender)
```
...
stargazer(m6, type = "html", covariate.labels = c("Age","Age Sq","Education","Female","Midwest","South","West","Female x Education"), single.row = TRUE, font.size = "footnotesize")
```
</small>
---
class: center, middle, inverse

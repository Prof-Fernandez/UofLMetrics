...
__We use Multinomial Logit or Probit__
...
__Models used are Poisson and Negative Binomial__

---
## Limited Dependent Variables and Economic Theory

The one key insight of limited dependent variables is how well it fits in with economic theory.

Suppose you have two products, A and B. Which one will you buy first?

Economic Theory says you buy the good that gives you the most utility. 
...
The error terms $e_A$ and $e_B$ represent the unobserved characteristics for the products that the econometrician cannot observed, but the consumer can. 
...
The product error terms are random variables. We cannot know definitely if $U_A > U_B$, but we can say what is the probability the consumer will choose product A over product B.
...
Remember, X is just data that we observed. $\beta$ is the parameter we are interested in. But the difference in the error terms is a random variable.
...
log odds  $$log \frac{Pr(Y=1)}{Pr(Y=0)}= log(s_1)-log(s_0)=X\beta$$.

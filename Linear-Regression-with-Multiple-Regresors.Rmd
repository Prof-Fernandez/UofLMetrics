...
## Assumption A.1 Plot
```{r echo=FALSE}
include_graphics("https://slideplayer.com/slide/12910416/78/images/23/Least+squares+assumption+%231%3A+E%28u%7CX+%3D+x%29+%3D+0..jpg")
```
...
- The large sample variance of $\hat{\beta}_1$ is $$\sigma_{\hat{\beta}_1}^2 = \frac{1}{n}\frac{\sigma^2_u}{[Var(X_i)]^2}$$
...
Another implication of the variance of $\hat{\beta}_1$ $$\sigma_{\hat{\beta}_1}^2 = \frac{1}{n}\frac{\sigma^2_u}{[Var(X_i)]^2}$$
...
$$\sigma_{\hat{\beta}_1}^2 = \frac{1}{n}\frac{\sigma^2_u}{[Var(X_i)]^2}$$
...
- __Positive Bias__: Our estimate of $\beta$ is too big.
- __Negative Bias__: Our estimate of $\beta$ is too small.
...
Suppose the true model is $$y_i = \beta_0+\beta_1 X1_i + \beta_2 X2_i + u_i$$
and $X2 = \delta_0+\delta_1 X1+e$. However, we estimate the model $$y_i = \beta_0+\beta_1 X1_i + \epsilon_i$$ 
```{r echo=TRUE, eval=FALSE}
# Parameters set by the user
s <- 10000 #Number of simulated samples
n <- 1000  #Sample size
sigma <- 1

# Main equation
beta1 <- -0.5 #True constant
beta2 = 2 #True slope coefficient
beta3 = 1 #True slope coefficient
...
b_vec <- rep(0,s) #Initializing the vector of estimated coefficients
Y_vec <- rep(0,s) #Initializing the vector of average prediction
for (i in 1:s) {  
  #Small sample
  u   <- rnorm(n, 0 ,sigma) #Error term in the main equation
  eps <- rnorm(n, 0, sigma) #Error term in the auxiliary equation
  X2  <- rnorm(n, 0, sigma) #Included Regressor
  X3  <- delta1 + delta2*X2 + eps #Omitted regressor
  Y   <- beta1 + beta2*X2 + beta3*X3 + u #Dependent variable
  model_miss <- lm(Y ~ X2)
  
  b_vec[i] <- model_miss$coefficients[2] #OLS estimates of misspecified model
  Y_vec[i] <- mean(predict(model_miss)) #Mean predicted values
}
# Histogram of estimates
hist(b_vec, breaks=20, col='gold3', 
     freq = FALSE,
     xlim=c(0, 3),
     main=paste('OLS coefficient: ',s,' samples of ',n, ' observations'),
     xlab = 'Coefficient values')
abline(v=beta2, col="darkgreen",lwd=4) #Vertical line at true parameter value
lines(density(b_vec),col="red",lwd=2)
```
...
beta1 <- -0.5 #True constant
beta2 = 2 #True slope coefficient
beta3 = 0 #True slope coefficient
...
het.se <- vcovHC(regress.results) #Robust SE's
...
None of the data available has $STR_i \le 12$ 

therefore $NVS_i$ is always equal to $1$.
...
It differs from perfect multicollinearity it that they don't have to be exact linear functions of each other.
...
## Load the data 
.pull-left[
```{r results='asis',warning=FALSE,message=FALSE, eval=FALSE, echo=TRUE}
library("AER")
library("lattice")
library("gtsummary")
data("CPSSW8")
tbl_summary(CPSSW8, by = gender)
```
]
.pull-right[
```{r results='asis',warning=FALSE,message=FALSE, eval=TRUE, echo=FALSE}
library("AER")
library("lattice")
library("gtsummary")
data("CPSSW8")
...
stargazer(m1,type="html", covariate.labels = "Female", style = "apsr", omit.table.layout = "n", star.char = c("c","b","a"), notes = c("c","b","a"))
```
</small>
]
...
stargazer(m1,m2,type="html",omit.stat = c("f","ser"), single.row = TRUE, style = "apsr", star.char = c("c","b","a"), notes = c("c","b","a"),omit.table.layout = "n", covariate.labels = c("Female","Age","Education"))
```
</small>
---
## Quadratic function
...
stargazer(m1,m2,m3,type="html",omit.stat = c("f","ser"), covariate.labels = c("Female","Age","Age Sq","Education"), style = "apsr", omit.table.layout = "n", star.char = c("c","b","a"))
```
...
CPSSW8$age2=CPSSW8$age*CPSSW8$age
m3 = lm(earnings ~ factor(gender)+age+age2+education, data=CPSSW8)
stargazer(m1,m2,m3,type="html",omit.stat = c("f","ser"), style = "apsr", omit.table.layout = "n", star.char = c("c","b","a"), single.row = TRUE, covariate.labels = c("Female","Age","Age Sq","Education"))
```
</small>
---
## Interaction term 
...
stargazer(m1,m2,m3,m4,type="html",omit.stat = c("f","ser"), single.row = TRUE, covariate.labels = c("Female","Female*Educ","Age","Age Sq","Education"), style = "apsr", omit.table.layout = "n", star.char = c("c","b","a") )
```
</small>
---
...
stargazer(m4,m5,type="html",covariate.labels = c("Age","Age Squared","Education","Female", "MidWest","South","West","Education*Female"),omit.stat = c("f","ser"), style = "apsr", omit.table.layout = "n", star.char = c("c","b","a"), single.row=TRUE)
```
</small>
---
## Log Transformation
...
One method used to insure that we have a positive dependent variable is to transform the dependent variable by taking the natural log. 
...
stargazer(m4,m5,m6,type="html", covariate.labels = c("Age","Age Squared","Education","Female", "MidWest","South","West","Education*Female"),omit.stat = c("f","ser"), style = "apsr",omit.table.layout = "n", star.char = c("c","b","a"), single.row=TRUE)
```
</small>
---
class: center, middle, inverse
...
stargazer(reg1,reg2,reg3,reg4, type = "html", style = "apsr", omit.table.layout = "n")
```
---
...
coeftest(regress.results.fe.only)

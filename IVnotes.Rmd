...
Remember that an omitted variable bias is present when
1. An omitted variables W explains Y
2. The omitted variable is correlated with X.

I will show you how big of a difference this can have on your estimates.
---
## OVB example 1
rho = 0.6
```{r}
...
rho <- 0.6; mu1 <- 1; s1 <- 2; mu2 <- 1; s2 <- 8
# Parameters for bivariate normal distribution
mu <- c(mu1,mu2) # Mean
bvn1 <- as.data.frame(mvrnorm(N, c(1,1),matrix(c(s1^2, s1*s2*rho, s1*s2*rho, s2^2),2)))
colnames(bvn1) <- c("X","W")
u1 <- rnorm(N,sd=4)
bvn1$y <- 12+2*bvn1$X-4*bvn1$W
summary(lm(y~X,data = bvn1))
```
...
Measurement error can occur for several reasons. 

1) Recall error: sometimes people just do not remember the exact number. Could you tell me how much your take home pay is? How many minutes you slept last night? How much you weigh?

2) Lying: Maybe you know the true value, but you do not want to report it so you make up a number. Again, think of income, weight, and height. 

3) Data Entry error: Sometimes a finger slips or a number is transposed.
...
Measurement error in Y does not tend to have a large consequence.
Suppose we observe $y^*$ and $y* = y + v$, where $y$ is the true value and $v$ is a random independent error.

If the true regression equation is $$y = \beta_0+\beta_1X+e$$, but we use $y*$ instead of $y$, then the regression model changes to 
$$\begin{align} y^* &= \beta_0+\beta_1X+e \\ y+v &= \beta_0+\beta_1X+e \\ y &= \beta_0+\beta_1X+e-v \end{align}$$
Consequence, our regression error has a larger variance.
---
## Measurement error in X
If X has measurement error, then we will have bias in our estimate.
Recall, $\beta_1=cov(x,y)/var(x)$ in the simple linear regression case. 
Consider, the following regression. $$y=\beta_0+\beta_1X^*+u$$ where $X^*=X+v$. This means our estimate of $$\begin{align}\hat{\beta_1}&=\frac{COV(X^*,y)}{Var(X^*)}=\frac{COV(X+v,y)}{Var(X+v)} \\ &=\frac{COV(X,y)+COV(y,v)}{Var(X)+Var(v)} \\ &=\beta_1\frac{\sigma^2_x}{\sigma^2_x+\sigma^2_v} \end{align}$$
The result is an attenuation bias. That is, the estimated parameter is biased toward zero. As the measurement error, $\sigma^2_v$, becomes smaller the bias goes away.
---
class: middle, center, inverse

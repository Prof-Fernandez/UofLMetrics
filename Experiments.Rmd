...
$$E[Y_{1i}] - E[Y_{0i}] \\ \text{Health w/ Ins} - \text{Health w/o Ins}$$
...
The Naive Comparison doesn't consider that people endogenously choose to have health insurance. Health Insurance is not randomly assigned so there could be a bias.
...
In our example, what this means is that if we were to take someone randomly from the control group (i.e. no insurance) and give them insurance, then we would expect them to have the same benefit, on average, as those people in the treatment group.
...
We start with a single group of people and randomly assign them to two groups, treatment and control. 
Random assignment insures that the two groups look a like initially (i.e. same average age, education, percent female). The only difference between the groups at the end of the experiment should be due to the treatment
---
## Random Assignment
...
$$E[Y_{1i} - Y_{0i}|D_i = 1] = E[Y_{1i} - Y_{0i}|D_i = 0] = E[Y_{1i} - Y_{0i}]$$ This relationship now holds. So the naive comparison works! This holds true only for randomized control trials.

---
# Break for Perfect Doctor
...
Experiments provide a very transparent and simple empirical strategy. They solve the _selection bias_ problem. However, there are a number of potential problems:
  ...
  3. [Hawthorne effect](https://youtu.be/o4XX90lqT6E)
